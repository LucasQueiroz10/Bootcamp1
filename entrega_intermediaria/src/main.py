...
from api import obter_cotacao_dolar


def exibir_cabecalho():
    print("\n" + "=" * 45)
    print("       💸  GASTE MENOS CLI  v2.0.0")
    print("=" * 45)
...
def exibir_menu():
    ...
    print("  [6] Converter total para dólar")
    print("  [0] Sair")
    print("-" * 45)
...
def exibir_gastos(dados):
    ...
    if dados["limite"] is not None:
        ...
        if ultrapassou:
            print("\n  🚨  ATENÇÃO: Você ultrapassou o seu limite!")
        else:
...
def fluxo_total(dados):
    ...
    if dados["limite"] is not None:
        ...
        print("    ℹ️  Nenhum limite definido.")


def fluxo_conversao(dados):
    total = calcular_total(dados)

    if total == 0:
        print("\n⚠️  Nenhum gasto cadastrado para converter.")
        return

    try:
        cotacao = obter_cotacao_dolar()

        convertido = total / cotacao

        print(f"\n💵 Cotação atual do dólar: R$ {cotacao:.2f}")
        print(f"💰 Total convertido: US$ {convertido:.2f}")
